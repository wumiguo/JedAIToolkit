...
public class GroupLinkage extends AbstractEntityMatching {
    ...
    private List<AbstractModel[]> getModels(List<EntityProfile> profiles) {
        ...
        for (EntityProfile profile : profiles) {
            ...
            AbstractModel[] newModel = Arrays.copyOf(model, counter);
        	ModelsList.add(profCounter++, newModel);
    ...
    public double[][] getSimilarityMatrix(Comparison comparison) {
   	 if (isCleanCleanER) 
   	 {
   		int s1 = entityModelsD1.get(comparison.getEntityId1()).length;
    	int s2 = entityModelsD2.get(comparison.getEntityId2()).length;
    	double[][] modelsim = new double[s1][s2];
    	for (int i=0; i<s1; i++) {
    		for (int j=0; j<s2; j++) {
            	 modelsim[i][j]= entityModelsD1.get(comparison.getEntityId1())[i].getSimilarity(entityModelsD2.get(comparison.getEntityId2())[j]);
    		}
    	}
    	return modelsim;	
   	 }
   	 ...
   	 {
    	int s1 = entityModelsD1.get(comparison.getEntityId1()).length;
    	int s2 = entityModelsD1.get(comparison.getEntityId2()).length;
    	double[][] modelsim = new double[s1][s2];
    	for (int i=0; i<s1; i++) {
    		for (int j=0; j<s2; j++) {
            	 modelsim[i][j]= entityModelsD1.get(comparison.getEntityId1())[i].getSimilarity(entityModelsD1.get(comparison.getEntityId2())[j]);
            	 //System.out.print(modelsim[i][j]+" ");
    ...
    public double getSimilarity(double[][] simMatrix) {
    	...
    	for (int i=0; i<rows; i++) graph.addVertex("a"+i);
    	for (int j=0; j<columns; j++) graph.addVertex("b"+j);
    	...
    	for (int i=0; i<rows; i++)
    		for (int j=0; j<columns; j++)
    		{
    			if (graph.containsEdge("a"+i, "b"+j))
    			{
    				DefaultWeightedEdge e = graph.getEdge("a"+i, "b"+j);
...
//    	System.out.println("prin");
		...
		{
//    		System.out.print(sortEdgePosHelp[0][j]+" ");
//    		System.out.print(sortEdgePosHelp[1][j]+" ");
//			System.out.println(sortEdgeHelp[j]);
		}
    	QuicksortEdges(sortEdgeHelp, sortEdgePosHelp, 0, edgeCounter-1);
//    	System.out.println("sorted");
    	double nominator = 0;
    	double denominator = rows+columns; //m1+m2
    	for (int j=0; j<edgeCounter; j++)
		{
...
//    			System.out.println(sortEdgeHelp[j]);
    ...
    public void swap(double array[], int pos[][], int index1, int index2) 
