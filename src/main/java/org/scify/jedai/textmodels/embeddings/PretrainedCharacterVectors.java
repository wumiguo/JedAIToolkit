...
public class PretrainedCharacterVectors extends PretrainedVectors {
    ...
    void addCharacterVector(Double[] cvec, Double[] aggrVec){
        for(int i=0;i<dimension;++i)
            aggrVec[i] += cvec[i];
    }
    Double[] createCharacterWordVector(String token){
        Double[] char_word_vector = getZeroVector();
        ...
        if (num_token_chars == 0) return null;
        // normalize
    ...
    public void updateModel(String text) {
        ...
        for (String token : tokens){
            Double[] wordVector = createCharacterWordVector(token);
            if (wordVector != null) addVector(wordVector);
            localUpdates ++;
        }
